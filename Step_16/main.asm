...
wait_adc:
	...
	rjmp OUT_Updat
	; Включаем атоматическое изменение яркости
	clr r16
	out OCR1AH, r16
	out OCR1AL, r17
OUT_Updat:
...
BTN_NEW:
	...
	rjmp OUT_BN
START_PR:
	rcall BRIGHT_FLAG ; Вот запуск проги
OUT_BN1:
...
BRIGHT_FLAG:
	sbrc rFlags, rFlag_AvtoBr	; Пропустить следующую команду если бит в регистре очищен
	rjmp OUT_BRF1
	sbr rFlags, (1<<rFlag_AvtoBr) ; устанавливаем флаг
	rjmp OUT_BRF
OUT_BRF1:
	cbr rFlags, (1<<rFlag_AvtoBr) ; сброс флага
	push r16
	push r17
	; Возврат яркости к дефолтной
	ldi r16,210
	clr r17
    out OCR1AH, r17
	out OCR1AL, r16
