 ...
 */ 
 TIMER0_ISR:
    ...
    ldi  rBtn_Index, 0
FIND_IDX:
    sbrc rBtn_now, 3 ; Пропустить следующую команду если бит в регистре очищен
    rjmp BTN_SAVE
    inc  rBtn_Index
    lsl  rBtn_now
    rjmp FIND_IDX
...
CHANGE_MO:
	...
	cpi R16,8 ; проверяем нахождение в режиме редактирование регистра 10Н
	BREQ Wr_Re		; Перейти если равно
    rcall SAVE_TO_RTC      ; Если были в редактировании времени - сохраняем
	rjmp Out_CHM
Wr_Re:
	rcall WRITE_REGISTER_10H	; выполняем запись в регистр 10Н
	cbr rFlags, (1<<rFlag_Reg10h) ; сбрасываем флаг
	clr r16				; делаем сброс режимов работы часов
	sts edit_mode, r16
...
CHANGE_ED:
	lds r16, edit_mode
    inc r16
    cpi r16, 9             ; 8 параметров + 1 выход
    brlo ED_ST
    clr r16
ED_ST:
    sts edit_mode, r16
...
PLUS:
	lds  r16, edit_mode
    tst  r16
    breq PL_BRIGHT           ; Если не в режиме настройки — Увеличиваем яркость
	cpi r16,8
	breq Pl_CRUS		   ; Перейти если равно, работаем с изменение регистра кварца
...
Pl_CRUS:
	...
	rjmp  Pl_DONE       
PL_BRIGHT:
	sbrs rFlags, rFlag_AvtoBr	; Пропустить следующую команду если бит в регистре установлен
	rcall BRIGHT_UP
Pl_DONE:
...
MINUS:
	lds  r16, edit_mode
    tst  r16
    breq MI_BRIGHT           ; Если не в режиме настройки — Уменьшаем яркость
	cpi r16,8
	breq MI_CRUS		   ; Перейти если равно, работаем с изменение регистра кварца 
...
BRIGHT_UP:
	in r16, OCR1AL
	in r17, OCR1AH
	cpi r16, 15   ; Проверка нижней границы (макс. яркость)
    brlo LIMIT_UP    ; Перейти если меньше Не уменьшаем дальше
    subi r16, 5  ; Уменьшаем значение OCR1A (свет станет ярче)
	rjmp BRU_OUT
LIMIT_UP:
	ldi r16, 10
BRU_OUT:
...
BRIGHT_DOWN:
	in r16, OCR1AL
	in r17, OCR1AH
	cpi r16, 250	; Проверка верхней границы (мин. яркость)
    brsh LIMIT_DOWN    ; Перейти если больше или равно  Не увеличиваем дальше
    subi r16, -5	; Увеличиваем значение OCR1A (свет станет тусклее)
	rjmp BRD_OUT
LIMIT_DOWN:
	ldi r16, 250
BRD_OUT:
...
Pl_CRUSTAL:
	...
	lds   r16, i2c_crystal
    cpi   r16, 127
    breq  inc_done           ; если уже максимум, не увеличиваем
    inc   r16
    sts   i2c_crystal, r16
inc_done:
	pop r16	
...
MI_CRUSTAL:
	push r16
	lds   r16, i2c_crystal
    cpi   r16, -128          ; сравниваем с -128 (0x80)
    breq  dec_done
    dec   r16
    sts   i2c_crystal, r16
dec_done:
