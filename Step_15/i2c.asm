...
READ_DATETIME_FULL:
 ...
 ;=========== Чтение регистра смещения старения кварцевого резонатора ===========
 READ_REGISTER_10H:
	push r16
    ...
    ; Регистр 0x10 (Crystal Aging Offset (10h))
    ldi r16, 0x10
    rcall I2C_WRITE
	...
	sbr rFlags, (1<<rFlag_Reg10h) ; устанавливаем флаг

	pop r16
   ret

 ;=========== Запись в регистр смещения старения кварцевого резонатора ===========
 WRITE_REGISTER_10H:
	push r16
	...
	lds r16, i2c_crystal ; наше новое значение
    rcall I2C_WRITE
		
	rcall I2C_STOP
	
	; Теперь запускаем принудительное преобразование температуры и тем самым применения изменений в регистре 10H
    ; (если не хотим ждать 64 секунды)
    rcall I2C_START
    ldi   r16, 0xD0
    rcall I2C_WRITE
    ldi   r16, 0x0E          ; регистр управления
    rcall I2C_WRITE
    ; Читаем текущее значение регистра управления (опционально)
    rcall I2C_START
    ldi   r16, 0xD1
    rcall I2C_WRITE
    rcall I2C_READ_NACK      ; прочитали байт, он в TWDR
    ; Устанавливаем бит CONV (бит 5)
    in    r16, TWDR
    sbr   r16, (1<<5)        ; установить бит 5
    push  r16	; в стек
    ; Записать обратно
    rcall I2C_START
    ldi   r16, 0xD0
    ...
    rcall I2C_WRITE
    rcall I2C_STOP
	; бит CONV (бит 5) Обнулится после преобразования
	pop r16
   ret
