...
ret
 ...
 VALUE_UP:
	...
	breq VU_BRIGHT				; Если 0 — мы не в режиме настройки, регулируем яркость
...
VU_BRIGHT:
	...
	; 2. Проверяем, не вылетим ли за 255 перед сложением
    ldi r17, 250    ; 255 - 5
    cp r16, r17
    brlo M_ADD      ; Если меньше 250, можно смело прибавлять
    
    ldi r16, 125      ; Если дошли до предела, сбрасываем до 125
    rjmp VU_WRITE

M_ADD:
	ldi r17, 5
    add r16, r17    ; Прибавляем шаг
VU_WRITE:
	clr r17
    out OCR1AH, r17
	out OCR1AL, r16
...
VU_EXIT:
 ...
 VALUE_DOWN:
	...
	breq VD_BRIGHT				; Если 0 — мы не в режиме настройки, регулируем яркость
...
VD_BRIGHT:
    ...
    ldi r17, 10    ; 10 - 5
    cp r16, r17
    BRSH M_SUB      ; Если больше или равно 10, можно смело отнимать
